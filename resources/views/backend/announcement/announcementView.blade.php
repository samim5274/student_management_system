                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($announcement as $val)
                                <tr>
                                    <td>#</td>
                                    <td><a href="{{url('/view-announcement-specific/'.$val->id)}}">{{$val->title}}</a></td>
                                    <td>{{$val->description}}</td>                                    
                                    <td><img src="{{asset('/img/uploads/'.$val->image)}}" alt="Loading.." style="width:3rem; height:2.5rem;"></td>                                    
                                    <td class="d-flex justify-content-around">
                                      <a href="{{url('/announcement-edit/'.$val->id)}}" class="btn btn-warning btn-sm">Edit</a>
                                      <form action="{{url('/announcement-delete/'.$val->id)}}" method="GET" style="display:inline;">
                                          @csrf
                                          @method('DELETE')
